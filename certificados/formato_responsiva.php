...
$no_responsiva=$_POST['no_responsiva'];

                         $statement=$conexion->prepare('SELECT  MAX(id_responsiva) FROM responsiva'); 
                         $statement->execute();
                         $constancia=$statement->fetchAll();
                         foreach ($constancia as $fila) 
                         {    
                              $folio_responsiva=$fila[0]+1; 
                         }

$inicio=$folio_responsiva;
$pdf = new FPDF();
...
$folio_responsiva=$folio_responsiva-1;
$registrar_carta_responsiva=$conexion->prepare("INSERT INTO responsiva(id_responsiva,nombre_chofer,direccion,estado,no_licencia,servicio,modelo,folio_certificado)VALUES('$folio_responsiva','----','----','----','----','----','----','0')");
$registrar_carta_responsiva->execute();


//Registrar certificados en blanco
$blancos=$conexion2->prepare("INSERT INTO certificadosblancos(id,tipo,inicio,fin,fecha_registro,responsable)VALUES(null,'R','$inicio','$folio_responsiva',null,'$id_usuario')");
$blancos->execute();

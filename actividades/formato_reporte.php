...
foreach ($certificados as $fila) {
  ...
  foreach ($registros as $key) {
    ...
    $pdf->Cell(30, 3, utf8_decode($nombre_finca), 1, 0, 'C');
    $pdf->Cell(45, 3, utf8_decode($empleado), 1, 0, 'C');
    $pdf->Cell(22, 3, utf8_decode(substr($fecha_actividad, 8, 2) . "/" . substr($fecha_actividad, 5, 2) . "/" . substr($fecha_actividad, 0, 4)), 1, 0, 'C');
    $pdf->Cell(22, 3, utf8_decode(substr($fecha_registro, 8, 2) . "/" . substr($fecha_registro, 5, 2) . "/" . substr($fecha_registro, 0, 4)), 1, 0, 'C');
    $pdf->Cell(17, 3, utf8_decode($unidad), 1, 0, 'C');
    $pdf->Cell(17, 3, utf8_decode($precio), 1, 0, 'C');
    $pdf->Cell(19, 3, utf8_decode($cantidad), 1, 0, 'C');
    $pdf->Cell(19, 3, utf8_decode('$' . $costo), 1, 1, 'C');
  ...
  if (count($registros) > 0) {
    ...
    $pdf->Cell(136, 5, utf8_decode(""), 0, 0, 'C');
    $pdf->Cell(17, 5, utf8_decode("Total:"), 1, 0, 'C', 1);
    $pdf->Cell(19, 5, utf8_decode($total_cantidad), 1, 0, 'C', 1);


    $pdf->Cell(19, 5, utf8_decode("$" . $suma), 1, 1, 'C', 1);
    $pdf->SetFillColor(0, 0, 0);
  }
}
